...
import type { PubLoginFormIF, PubLoginFormRef, SignFieldItem, signTy } from "../../common/types";
...
// 默认邮箱后缀
const defaultEmailSuffixes = ["@qq.com", "@163.com", "@126.com", "@gmail.com", "@outlook.com"];
...
const renderFormItem = (item: SignFieldItem) => {
  ...
  if (item.type === "email" && !item.component) {
    ...
    return (
      <div key={item.field} style={{ display: "flex", gap: 8 }}>
        <Form.Item name={item.field} rules={emailRules} dependencies={[suffixKey]} style={{ flex: 1, marginBottom: 16 }}>
  ...
  return (
    <Form.Item name={item.field} key={item.field} rules={item.rules ?? rulesMap[item.type]}>
...
const mergeEmailValues = (values: Record<string, any>, content: SignFieldItem[]) => {
  const merged = { ...values };
      ...
      <div className={style.loginTechno}>
        <div className={rightActiveClass}>
          ...
          >
            ...
            <div>
              {signUpContent?.map((item) => renderFormItem(item))}
            </div>
          ...
          <Form
            ...
            onFinish={(v) => onSubmit(mergeEmailValues(v, signInContent), "signIn")}
          >
            <p className={style.title}>登录</p>
            <div>
              {signInContent?.map((item) => renderFormItem(item))}
            </div>
            <div style={{ width: "100%", flexDirection: "column", ...center }}>
              <p
